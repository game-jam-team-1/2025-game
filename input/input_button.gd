...
enum ButtonState {
	...
	RELEASED, ## If the button was released last frame and this frame.
...
var _state: ButtonState = ButtonState.RELEASED
...
func _init(new_valid_elements: Array[InputEvent] = []) -> void:
	valid_events = new_valid_elements
...
## This is usually only called by the [class InputManager]. Must be called with
## every input to ensure the button works properly.
func get_state_with_input(input: InputEvent) -> ButtonState:
	...
	if _state == ButtonState.RELEASED && input.is_pressed():
		_axis = 1.0
...
## Returns if this button is pressed.
...
## Gets the axis (between 0 and 1) of this button. Works with digital and analogue
## inputs.
func get_axis() -> float:
	return _axis
